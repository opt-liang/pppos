...
void create_tcp( void ){
    ...
    if( pcb ){
        ...
        tcp_connect( pcb, &ipaddr, 2003, tcp_Client_connected );
    }
...
//        printf( "------------------------------TCP STATE:%d----------------\r\n", pcb->state );
        ...
        if( pcb != NULL && pcb->state == 4 ){
            ...
            memset( data, 0, 32 );
            if( sprintf( data, "%d\r\n", i++ ) > 0 ){
                while( ERR_OK != tcp_write( pcb, data, strlen( data ), TCP_WRITE_FLAG_COPY ) ) osDelay(10);
                while( ERR_OK != tcp_write( pcb, "123456789abcdefghijklmnopqrstuvwxyz\r\n", strlen( "123456789abcdefghijklmnopqrstuvwxyz\r\n" ), TCP_WRITE_FLAG_COPY ) ) osDelay(10);
                printf( "%s\r\n", data );
            }
        }
